...
THIS TUTORIAL COMPLETED BY: Max Felberg
...
x <- 5
(x<-5) #forces it to print
x # or you just call the variable straight up to rpint 
```

```{r}
# objects can store more than one value in a one dimensional vector
my_vector <- c(1,4,5,6,8)
#c function stands for combine, saves it as like a list of numbers / vector really 
my_vector #calling the vector 

my_vector*2 #multiples each number by 2 
```
...
#install it once on the computer, but reopen it in R every time 
install.packages("palmerpenguins") #type in name of function in quotes 

install.packages("rmarkdown")

# Each time you reopen R however, you need to call the packages you're using fresh from the library
library(palmerpenguins)  #now you can use these functions 

#only load the packages you need, functions can have the same name across multiple packages 

# Then you can access the data frames and functions stored in the package :) 
penguins #now you can pull up the data and look at it, this is an object that contains a big ol spreadsheet 
...
library(tidyverse) #dont use quotes when calling the package 
...
```{r}
install.packages("fivethirtyeight") #installing the package!! 
library(fivethirtyeight) #loadin it 

fivethirtyeight:: #how to preveiw all the files in the package
```
...
```{r}
head(candy_rankings) #shows first 6 rows of dataset
```

```{r}
glimpse(candy_rankings)

#shows you each row and column and the TYPE of data being stored there 
#types: <lgl> logical 
```

```{r}
View(candy_rankings) #opens it in a new view tab all nice 
```
...
These pieces are layered into the plot with + signs. ggplot has its own syntax you need to use
...
#just says, i want to make a plot! basically just makes an empty space 
```

```{r}
ggplot(candy_rankings) +   #put the file you want to plot data from in ()
  aes(x = chocolate) # this says what to put on the axes, this is aesthetic 
```
...
#bar plots automatically tally it and takes a count
```
...
#now we've saved the plot as an object 

chocolate_bar #calling the object 
...
sweetness <- ggplot(candy_rankings) + 
  ...
  geom_point()   #geompoint gives us a scatterplot, plottin points baby 


#remember, you dont have to put anything in the geom_something parenthesis 


sweetness #calling the plot 
...
```{r}
sweetness +   #you can call a plot from before and add more arguments by throwing a + at the end
  geom_smooth(method = "lm")   #method and "lm" adds a best fit line, basic linear model 
```
...
sweetness <- ggplot(candy_rankings)+
  ...
  geom_boxplot()  #making a box plot with just sugar and chocolate 
...
sweethist <- ggplot(candy_rankings)+
  aes(x=sugarpercent)+
  geom_histogram()  #histogram only needs an x axis!!!!!! 
...
```{r}
fruityprice <- ggplot(candy_rankings)+ #selecting dataset
  aes(x=fruity, y=pricepercent)+ #choosing axis
  geom_boxplot()  #choosing type

fruityprice #fruity candies are cheaper?? 
```
...
sweetprice <- ggplot(candy_rankings)+
  ...
  geom_point() +#making it a point plot
  geom_smooth(method = "lm")  #making it show the linear relationship

sweetprice  #sweeter candies are a little more expensive!! 

```

```{r}
sugarwin <- ggplot(candy_rankings)+  #making the plot
  aes(x=sugarpercent, y=winpercent)+
  geom_point()+
  geom_smooth(method="lm") #adding linear relationship

sugarwin #calling the plot, sweeter candies also kinda win more 
```
